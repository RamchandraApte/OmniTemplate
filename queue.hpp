...
public:
  ...
  void push(const T& val){
    Base::push({val, ::min(val, min())});
  }
  const T& top() const {
    return Base::top().first;
  }
  T min() const {
    return Base::empty()?inf:Base::top().second;
  }
};
void test_minstack(){
  MinStack<ll> ms;
...
template<typename T>
class MinQueue: public QueueAdaptor<MinStack<T>> {
public:
  T min(){
    return ::min(this.get_left().min(), this.get_right().min());
  }
};
void test_minqueue() {
  MinQueue<ll> mq;
